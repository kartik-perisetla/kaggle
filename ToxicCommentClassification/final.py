...
def get_tf_idf_dict(text, tolerance=5):
    tf_vec = TfidfVectorizer(min_df=tolerance)
    return tf_vec.fit_transform(text), tf_vec

def extract_features(list_of_text, vectorizer= DictVectorizer(), is_binary=True):
    if is_binary:
        return vectorizer.fit_transform([get_binary_dict(text) for text in list_of_text]), vectorizer
    else:
        return get_tf_idf_dict(list_of_text)
...
def extract_test_features(list_of_text, vectorizer):
    ...
    return vectorizer.transform(list_of_text)
    
def train_on_features(class_label, train_features, train_labels, model_skeleton):
    model = copy.copy(model_skeleton)
    print("about to fit model:'"+model.__class__.__name__+"' on " + str(len(train_labels)) + " instances")
    model.fit(train_features, train_labels)
    print("fitting complete on model:'"+model.__class__.__name__+"' on " + str(len(train_labels)) + " instances")

    print("\n")
    print("*"*CHAR_REP + model.__class__.__name__ + "*"*CHAR_REP )
    # print model params
    # print("\nBest params for '"+ model.__class__.__name__ + "' for class '" + class_label + "' => " + str(model.best_params_))

    # commenting -> no need now as we are doing parameter sweep
    print("Cross-Validation Accuracy for '"+ model.__class__.__name__ + "' for class '" + class_label + "' => ")
...
def get_features(text_list, key=None, is_binary=True):
    ...
    """    
    if key is None:
        train_features, vectorizer = extract_features(text_list, is_binary=is_binary)
...
def train_on_file(train_file_name, model_skeleton):
    ...
    train_features, vectorizer = get_features(text_list, TRAIN_KEY, False)
...
def test_on_file(test_file_name, model_collection, vectorizer):
    ...
    print("test_on_file:about to extract features for test instances")
...
def write_to_submission_file(filepath, ids, results):
    writo = [['id'] + get_from_cache(CLASS_LABELS_KEY)]
...
def run(args):
    ...
    if not skip_train_flag:
        ...
        for score in scores:
            ...
            models = [CalibratedClassifierCV(svm.LinearSVC(C=1))]

            for model_skeleton in models:
                # print("Training '" + model_skeleton.estimator.__class__.__name__ + "' on file")
                print("Training '" + model_skeleton.__class__.__name__ + "' on file")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 5:
        ...
        print("python final.py <train_file_name> <test_file_name> <output_file_name> <skip_train_flag>")
